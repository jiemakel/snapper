angular.module('snapper').controller('MainCtrl', ($scope, $http, toastr, $document, $stateParams, $localStorage, prefixService, $q, sparql) ->
	...
	updateGraphs = () ->
			...
			GROUP BY ?graphIRI
			''',{timeout: canceler.promise}).then((response) ->
				$scope.graphs = response.data.results.bindings
			,handleError)
	getLineAfterPrefixes = (data) ->
		pos = 0
		i = 0
	...
	applyAutocompletion = (cm,data,completion) ->
		...
		if (!knownPrefixes[sf.prefix]?)
			appendPrefix(sf.prefix,sf.ns)
	getNodeFromPosition = (cm,cur) ->
	...
	getNextNode = (cm,pos) ->
		...
		while (token.type!="string" && token.type!="keyword" && token.type!="variable-2" && token.type!="string-2")
			...
			if (token.end==pos.ch-1)
				...
				token = cm.getTokenAt(pos)
		{ token: token, pos : pos }
	autocompleteHint = (cm, callback, options) ->
		cur = cm.getCursor()
		token = cm.getTokenAt(cur)
		...
		if (prefix?) # prefixed, first do lov search, then IRI substring search
			...
			$http.get("http://lov.okfn.org/dataset/lov/api/v2/autocomplete/terms",
				params: 
	...
	deleteCurrentSubjects = (cm) ->
		...
		if (!currentSubjects)
			...
			sparql.update($scope.sparulEndpoint,query).then((response) ->
				updateGraphs()
	...
	replaceCurrentSubjects = (cm) ->
		...
		if (!currentSubjects)
			...
			query = replaceAll(replaceAll(deleteResourceQuery.getValue(),"<IRIS>",currentSubjects.iris.join(" ")),"<GRAPHIRI>",$scope.graphIRI)
			sparql.update($scope.sparulEndpoint,query).then((response) ->
				sparql.post($scope.restEndpoint,getPrefixLinesFromData($scope.data)+currentSubjects.data,$scope.graphIRI).then((response) ->
					updateGraphs()
